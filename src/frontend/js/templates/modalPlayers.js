  ...
  const controllers = () => {
    ...
    function renderPlayerList() {
      ...
      playersNames.forEach((name, index) => {
        ...
        li.innerHTML = `<div><span class='players'>${
          index + 1
        }</span><p>${name}</p></div><span class="removePlayer">&times;</span>`;
      ...
      document.getElementById("remainingPlayersSpan").innerHTML =
        "Players: " + playersNames.length + "/50 "; // At the moment the max players are static
    }
    ...
    addButton.onclick = function () {
      ...
      if (playerName) {
        ...
        if (!playersNames.includes(playerName)) {
          ...
          document.getElementById("msg--err").innerHTML =
            "\u26A0  You cannot introduce repeated names!";
      } else
        document.getElementById("msg--err").innerHTML =
    ...
    unmuteBtn.onclick = function () {
      ...
      this.className =
        videoEl.muted == true
          ? "fas fa-volume-mute btn--mute off--red"
          : "fas fa-volume-off btn--mute";
    };
    ...
    exportBtn.addEventListener("click", function () {
      let players = JSON.parse(localStorage.getItem("playersNames")).map(
        (e) => e
      );
      if (players.length != 0) {
        let csvContent = "data:text/csv;charset=utf-8," + players;
